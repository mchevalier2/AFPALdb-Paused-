...
#' @export
#' @examples
#' Pella.init=Interpolation.init(sites=c("PEL-1-1","PEL-1-4a"),variable="ReconAI",time.range=c(0,50000),time.resol=100,NREP=1000,Citation_Keys=c("Lim_etal_2016","Lim_etal_2016"))

Interpolation.init <- function(sites,variable,time.range,time.resol,NREP=50000,Citation_Keys=rep("",length(sites))){
...
#' @export
#' @examples
#' Pella.init=MonteCarloInterpolation(Pella.init,exportData=TRUE)
...
#' @export
#' @examples
#' exportMonteCarloInterpolation(Pella.init)
...
exportMonteCarloInterpolation <- function(dat,saveData=getwd()){
    ...
    for(s in dat$Sites){
        ...
        cat("100%.\n")
        .openPDF(paste(saveData,"/",s,"_",dat$Variable,"_MonteCarlo.pdf",sep=""))
    }
}
...
#' @export
#' @examples
#' Pella.init=MonteCarloCentring(Pella.init,"PEL-1-1",c("PEL-1-4a",100,1200))

MonteCarloCentring <- function(dat,ref,overlap){
    overlap=matrix(overlap,ncol=3,byrow=TRUE)  ;  rownames(overlap)=overlap[,1]
    for(s in dat$Sites){
        if(s != ref){
            w=which(dat$XX <= as.numeric(overlap[s,3]) & dat$XX >= as.numeric(overlap[s,2]))
...
MonteCarloStacking <- function(dat,exportData=FALSE,saveData=getwd()){
    ...
    cat("Stacking\t0.. ")
    dat$StackMC=matrix(0,ncol=length(dat$XX),nrow=dat$NREP)
...
#' @param dat The classic Monte-Carlo object generated by Interpolation.init().
#' @param saveData Folder where to save the outputs (in the working folder by default).
#' @param vline List of position from where to draw a vertical dashed lines (the total xrange is divided in 10 equal intervals by default).
...
exportMonteCarloStacking <- function(dat,saveData=getwd(),vline=seq(min(dat$XX),max(dat$XX),length.out=11),XLAB="Age"){
    cat("Exporting\t0 .. ")
    pdf(paste(saveData,"/Stack_",dat$Variable,"_SpaghettiPlot.pdf",sep=""),width=3.54,height=5)  ;  {
        ...
        axis(2,lwd=1,lwd.ticks=1,cex.axis=0.8)
        cat("10 .. ")
        for(i in vline){    abline(v=i,lty=5,lwd=0.3)    }
        cat("20 .. ")
        ...
        legend("bottomright",legend=c("Stack",dat$Sites),col=c("Black",unlist(dat$COL)),cex=0.8,text.col=c("Black",unlist(dat$COL)),fill=c("Black",unlist(dat$COL)),border=c("Black",unlist(dat$COL)),bg="White")
        cat("40 .. ")
    ...
    dev.off()
    .openPDF(paste(saveData,"/Stack_",dat$Variable,"_SpaghettiPlot.pdf",sep=""))

    cat("80 .. ")
    rownames(dat$Stack)=c(dat$Variable,paste(dat$Variable,"smoothed"),"Nsites","30%inf","30%sup","50%inf","50%sup","80%inf","80%sup")
    cat("90 .. ")
    write.csv(cbind(dat$XX,t(dat$Stack)),paste(saveData,"/Stack_",dat$Variable,"_MonteCarlo.csv",sep=""),row.names=FALSE)
    cat("100%.\n")
}
